 ...
 *    | `6-xx`        |     | NG group reference values (X1 in the decoding formula), each of which is|
 *    |               |     | encoded using the number of bits specified in octet 20 of data          |
 *    |               |     | representation template 5.0. Bits set to zero shall be appended as      |
 ...
 *    |               |     | bits specified in octet 47 of data representation template 5.2. Bits set|
 *    |               |     | to zero shall be appended as necessary to ensure this sequence of       |
 *    |               |     | numbers ends on an octet boundary (see Note 5)                          |
 ...
 */
open class Grib2RecordDS2(length: Int, data: FloatArray) : Grib2RecordDS(length, data) {
	companion object {
		/**
		...
		internal fun readFromStream(gribInputStream: GribInputStream,
			...
			val groupLength = try {
				...
				return Grib2RecordDS2(length, data)
			}
