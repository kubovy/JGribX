...
class GribFile(gribInputStream: GribInputStream, parameterFilter: (String) -> Boolean = { true }) {
	...
	init {
		...
		while (gribInputStream.available() > 0) {
			count++
			try {
				val message = GribMessage.readFromStream(gribInputStream, parameterFilter)
				...
				message.records.forEachIndexed { index, record ->
					...
					Logger.info("\t\tParameter: ${record.productDefinition.parameter.code}" +
							" (${record.productDefinition.parameter.description})")
					Logger.info("\t\tLevel: ${record.productDefinition.level?.code ?: ""}" +
							" (${record.productDefinition.level?.description ?: ""})")
				}
