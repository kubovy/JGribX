 ...
 *    | `5`           | `1` | Number of Section: `4`                                                  |
 *    | `6-ww`        |     | First value(s) of original (undifferenced) scaled data values,          |
 *    |               |     | followed by the overall minimum of the differences. The number of       |
 *    |               |     | values stored is 1 greater than the order of differentiation, and the   |
 *    |               |     | field width is  described in octet 49 of Data Representation            |
 *    |               |     | Template 5.3                                                            |
 *    | `[ww+1]-xx`   |     | NG group reference values (X1 in the decoding formula), each of which   |
 *    |               |     | is encoded using the number of bits specified in octet 20 of Data       |
 *    |               |     | Representation Template 5.0. Bits set to zero shall be appended where   |
 *    |               |     | necessary to ensure this sequence of numbers ends on an octet boundary. |
 *    | `[xx+1]-yy`   |     | NG group widths, each of which is encoded using the number of bits      |
 *    |               |     | specified in octet 37 of Data Representation Template 5.2. Bits set to  |
 *    |               |     | zero shall be appended as necessary to ensure this sequence of numbers  |
 *    |               |     | ends on an octet boundary.                                              |
 *    | `[yy+1]-zz`   |     | NG scaled group lengths, each of which is encoded using the number of   |
 *    |               |     | bits specified in octet 47 of Data Representation Template 5.2. Bits    |
 *    |               |     | set to zero shall be appended as necessary to ensure this sequence of   |
 *    |               |     | numbers ends on an octet boundary. (see Note 14 of Data Representation  |
 *    |               |     | Template 5.2)                                                           |
 *    | `[zz+1]-nn`   |     | Packed values (X2 in the decoding formula), where each value is a       |
 ...
 */
class Grib2RecordDS3(length: Int, data: FloatArray) : Grib2RecordDS(length, data) {
	companion object {
